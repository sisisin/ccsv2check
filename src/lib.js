...
function getCsvPath() {
  if (process.argv.length !== 3) {
    throw new Error('Runtime error. Invalid command line args.');
  }
  if (!process.env['CCSV_DIR']) {
    throw new Error('Runtime error. Invalid environment variable. "CCSV_DIR" is required.');
  }
...
function parse(rawCsv) {
  ...
  const unknowns = rows.filter(row => row.indexOf('UnKnown') === 0).map(row => UnKnown.parse(row));
...
function toRenderString({ circles, colors }) {
  ...
  function toCsvString(circle) {
    return [
      colors.get(circle.colorNumber).colorName,
      circle.spacePrefix,
      circle.spaceNum,
      circle.spaceSuffix,
      circle.circleName,
